      ...
      <div className="flex items-center bg-white rounded-2xl shadow-lg border border-gray-200/50 overflow-hidden min-h-[48px] md:min-h-[56px] lg:min-h-[64px] max-w-2xl md:max-w-3xl lg:max-w-4xl xl:max-w-5xl gap-0 md:gap-0">
        ...
        <Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
          <PopoverTrigger asChild>
            <Button
              className="flex items-center px-1 md:px-1 py-2 md:py-3 hover:bg-gray-50 transition-colors min-w-[10px] md:min-w-[14px] lg:min-w-[16px] outline-none cursor-pointer"
            ...
            >
              <Calendar
                className={`w-5 h-5 mr-2 md:mr-3 ${
                  selectedDateRange?.from ? 'text-blue-500' : 'text-gray-400'
                }`}
              />
            </Button>
          </PopoverTrigger>
          <PopoverContent
            className="w-auto p-0 bg-white/80 backdrop-blur-sm"
            align="start"
            onMouseEnter={() => {
